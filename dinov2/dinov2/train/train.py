...
def do_train(cfg, model, resume=False):
    ...
    mask_generator = MaskingGenerator(
        input_size=(img_size // patch_size, cfg.crops.time_block_in_mins),
        max_num_patches=int(0.5 * img_size // patch_size),
    )

    data_transform = TrajectoryDataAugmentationDINO(
        cfg.crops.global_crops_size,
        cfg.crops.local_crops_scale,
        cfg.crops.local_crops_number,
        cfg.crops.num_4hr_slots
    )
    ...
    collate_fn = partial(
        ...
        mins_per_slot = cfg.ibot.time_block_in_mins,
        n_tokens=n_tokens,
        mask_generator=mask_generator,
        dtype=inputs_dtype,
    )

    # Inside do_train function, after loading the dataset
    ...
    for data in metric_logger.log_every(
        ...
        # modified for testing. REMOVE IT.
        loss_dict_reduced = {
            k: (v.item() / distributed.get_global_size() if not torch.isnan(v) else -1) 
