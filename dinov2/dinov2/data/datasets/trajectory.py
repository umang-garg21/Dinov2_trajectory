...
from .extended import ExtendedTrajectoryDataset
...
class Trajectory(ExtendedTrajectoryDataset):
    ...
    @property
    def split(self) -> "Trajectory.Split":
        return self._split
    ...
    def _get_entries(self) -> List[Tuple[str, int]]:
        """
        Collects the paths of `.npy` files and their class index.
        """
        if self._entries is None:
            dataset_root = os.path.join(self.root, self._split.value)  # Define the split directory (train/val/test)
            
            # Collect all .npy files in the dataset_root directory
            npy_files = [f for f in os.listdir(dataset_root) if f.endswith('.npy')]

            # For the purpose of this, we assume all files are in the same class.
            # Modify this as necessary if your files are organized differently.
            self._entries = [(os.path.join(dataset_root, file_name), 0) for file_name in npy_files]
    ...
    def get_npy_data(self, index: int) -> np.ndarray:
        ...
        npy_file_path = entries[index][0]  # Assuming (file_path, class_index) in _entries
        return np.load(npy_file_path)
    
    def get_target(self, index: int) -> Any:
        """
        Returns the target label for the given index.
        """
        _, class_index = self._get_entries()[index]
        return class_index
    ...
    def _dump_entries(self) -> None:
        split = self.split
        if split == Trajectory.Split.TEST:
            dataset = None
